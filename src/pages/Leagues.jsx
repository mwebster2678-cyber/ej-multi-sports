...
export default function Leagues() {
  ...
  async function fetchLeagues() {
    ...
    const { data: leagueData } = await supabase
      ...
      .in('status', ['launching', 'in_progress', 'finished'])
      .order('id', { ascending: true })

    setActiveLeagues((leagueData || []).filter(l => l.status !== 'finished'))
    setPastLeagues((leagueData || []).filter(l => l.status === 'finished'))
  ...
  function LeagueCard({ league, past }) {
      ...
      >
        <div className="p-4 flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            {past && <span className="inline-block mt-1 text-[10px] font-semibold uppercase tracking-wide text-gray-400 bg-gray-100 px-1.5 py-0.5 rounded">Finished</span>}
            {!past && league.status === 'launching' && <span className="inline-block mt-1 text-[10px] font-semibold uppercase tracking-wide text-amber-700 bg-amber-100 px-1.5 py-0.5 rounded">Launching Soon</span>}
          </div>
        ...
        {loading ? (
          ...
          <>
            {activeLeagues.map(league => (
              <LeagueCard key={league.id} league={league} past={false} />
            ))}
